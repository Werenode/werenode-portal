import {Drawer, Grid} from "@material-ui/core";
import Typography from "@material-ui/core/Typography";
import InfoIcon from "@material-ui/icons/Info";
import ElectricalServicesIcon from "@material-ui/icons/ElectricalServices";
import QrCodeScannerIcon from '@material-ui/icons/QrCodeScanner';
import {Connector} from "./Wizard";
import React, {useState} from "react";
import IconButton from "@material-ui/core/IconButton";
import {TicketPDFGenerator} from "./TicketPDFGenerator";
import {EVSE_LINK} from "../../utils/evse-smart-contract";

const styles = {
    evseInformations: {
        marginBottom: '15px',
        display: 'flex',
        alignItems: 'center',
    },
    subtitle2: {
        position : 'absolute',
        top : '-11px',
        paddingLeft : '8px',
        paddingRight : '8px',
        color: '#00B1A5',
    },
    h5Style: {
        marginBottom: '15px',
        display: 'flex',
        alignItems: 'center'
    },
};


export const EvseInfo = ({data}) => {
    const [open, setOpen] = useState(false);

    const toggleDrawer = () => {
        setOpen(open => !open);
    };

    return(
        <Grid container width='70%' margin='auto' direction='column' padding='10px'>
            <Grid item container direction='column'>
                <Typography variant='h5' component='div' sx={styles.evseInformations}>
                    <InfoIcon sx={{margin: '0px 8px  0px 0px'}}/>EVSE Information
                </Typography>
                <Grid item border='1px solid #34383e' position='relative' margin='15px 0px'>
                    <Typography variant='subtitle2' style={styles.subtitle2}>
                        Supervision
                    </Typography>
                    <div style={{padding: '10px',}}>{data.setting.supervision.type}</div>
                </Grid>
                <Grid item border='1px solid #34383e' position='relative' margin='15px 0px'>
                    <Typography variant='subtitle2' sx={styles.subtitle2}>
                        Gps Coordinates
                    </Typography>
                    <div style={{padding: '10px',}}>{data.setting.gps || <span>No Gps coordinates</span>}</div>
                </Grid>
                <Grid item border='1px solid #34383e' position='relative' margin='15px 0px'>
                    <Typography variant='subtitle2' sx={styles.subtitle2}>
                        Qr Code
                    </Typography>
                    <div style={{padding: '10px',}}>
                        <IconButton onClick={toggleDrawer}><QrCodeScannerIcon /></IconButton>
                    </div>
                    <Drawer anchor='right' open={open} onClose={toggleDrawer}>
                        <TicketPDFGenerator evseName={data.setting.id} evseLink={EVSE_LINK}/>
                    </Drawer>
                </Grid>
            </Grid>
            <Grid item container maxHeight='400px' direction='column' sx={{overflowY: 'auto'}} flexWrap="wrap">
                <Typography variant='h5' component='div' sx={styles.h5Style}>
                    <ElectricalServicesIcon sx={{margin: '0px 8px  0px 0px'}} /> Connectors
                </Typography>
                {
                    data.setting.connectors.length > 0 ?
                        data.setting.connectors.map((x,i) =>
                            <Grid item margin='10px 0px' key={`index-${i}`}>
                                <Connector
                                    key={"connector" + i}
                                    identifier={i}
                                    rmConnector={null}
                                    editConnector={null}
                                    supervisiontype={data.setting.supervision.type}
                                    data={data.setting.connectors[i]}
                                    hideButtons={true}
                                />
                            </Grid>
                        ) : <span>No connectors</span>
                }
            </Grid>
        </Grid>)
}
