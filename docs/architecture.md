...
The werenode-portal is a web interface for managing EVSE and providing documentation for developers and users. It is built with [Docusaurus](https://docusaurus.io/) to ensure easy navigation, maintainability, and scalability.
...
1. **User Interaction**: A user interacts with the mobile app, the werenode-portal, the Member App, or the DEFev web application to find a charging station or manage a session.
2. **API Request**: The interaction triggers an API request to the backend to retrieve EVSE data or execute a charging action.
3. **EVSE Manager**: The EVSE Manager automates blockchain transactions, interacting with the smart contracts to handle payment.
4. **Blockchain Transaction**: If payment is required, the user interacts with the integrated wallet (MetaMask or other supported wallets) to execute a blockchain transaction.
5. **EVSE Control**: The backend communicates via the OCPP layer or MQTT layer to start or stop the charging process.
6. **Data Update**: Charging session data is updated in the database and, where relevant, logged on the blockchain.
